...
class UMAPPlotter:
    ...
    def create_plot_with_border(
        ...
        ax.set_title(title, fontproperties=font_prop, fontsize=25, fontweight="extra bold")
    ...
    def plot_umap_with_user(
        ...
        *,
        ...
        if max_background_samples_per_class is not None:
            background_Umap = UMAPPlotter.reduce_background_umap_samples(
                umap_background_csv,
        ...
        # ---- sampling + smoothing ----
        mix_res = UMAPPlotter.sample_and_smooth_embeddings(
        ...
        if {"umap_x", "umap_y"}.issubset(background_Umap.columns):
            if {"scale_x", "scale_y"}.issubset(background_Umap.columns):
                ...
                mx, cx = UMAPPlotter._fit_linear_scale(background_Umap["umap_x"].values, background_Umap["scale_x"].values)
                my, cy = UMAPPlotter._fit_linear_scale(background_Umap["umap_y"].values, background_Umap["scale_y"].values)

                background_Umap_plot = background_Umap.copy()
                # apply same mapping to user
                mix_df_umap["scale_x"] = UMAPPlotter._apply_linear_scale(mix_df_umap["umap_x"].values, mx, cx)
                mix_df_umap["scale_y"] = UMAPPlotter._apply_linear_scale(mix_df_umap["umap_y"].values, my, cy)
                use_scaled = True
        ...
        fig = UMAPPlotter.create_plot_with_border(
        ...
        if annotate:
            UMAPPlotter.add_text_annotations(
                ax=ax,
                user_data=mix_df_umap,
                x_col=x_col,
                y_col=y_col,
        ...
        image_base64 = None
        if redis_key:
            image_base64 = self.save_plot_to_redis(fig, redis_key)
        
        if output_path:
            UMAPPlotter.save_plot_properly(fig, output_path)
