...
#### 🌍 Share the frontend with ngrok (Docker-only)
1. Set `ENABLE_NGROK_FRONTEND=true` and provide `NGROK_AUTHTOKEN` in `.env` (free accounts support a single tunnel).
2. Start the stack with `./docker-start.sh`. The script automatically enables the `ngrok-frontend` service and prints the generated HTTPS URL.
3. The URL is stored in `.env` as `PUBLIC_FRONTEND_URL`, and `scripts/show-ngrok-urls.sh --update-env --reload-backend` can be rerun anytime to refresh it.
4. The backend reads `PUBLIC_FRONTEND_URL` during startup and automatically whitelists it for CORS. Backend and dashboard remain private on the Docker network.
...
- `ENABLE_NGROK_FRONTEND` - Set to `true` to tunnel only the frontend container with ngrok
- `NGROK_AUTHTOKEN` - ngrok auth token (required when enabling the tunnel)
- `PUBLIC_FRONTEND_URL` - Cached HTTPS URL generated by ngrok; auto-whitelisted by backend CORS
- `PUBLIC_BACKEND_URL` - Optional HTTPS endpoint for the backend API (set when using a dedicated backend tunnel)

## 🌐 Access URLs
Once running, access the application at (ports configurable via `.env`):
...
docker compose --profile sharing down && docker compose --profile sharing up -d